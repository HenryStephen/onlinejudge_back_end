...
public class SubmissionConsumer {
    ...
    public void createSubmission(SubmissionVO submissionVO) throws Exception {
        ...
        // 更新题目的解决人数和提交数量
        // 查出来具体的关联信息
        CompetitionProblem competitionProblem = competitionProblemServiceApi.getByCompetitionIdAndDisplayId(submission.getSubmissionContestId(),submission.getSubmissionProblemId());
        Integer solvedNumber = competitionProblem.getSolvedNumber();
        Integer submitNumber = competitionProblem.getSubmitNumber();
        competitionProblem.setSubmitNumber(submitNumber+1);
...
//        userProblemServiceApi.saveOrUpdate(userProblem);
    ...
    private Integer judgeResponseStatus(List<JudgeResponse> judgeResponseList){
        ...
        if(judgeResponseList != null){
            ...
            for(JudgeResponse judgeResponse : judgeResponseList){
                if(judgeResponse.getResult().equals(JudgeResult.SUCCESS.getCode())){
                    // 如果有正确的，则计数
                    successCount++;
                }else{
